...
import com.sample.libs.image.core.assist.ImageScaleType;
import com.sample.libs.image.core.assist.ImageSize;
import com.sample.libs.image.core.assist.ViewScaleType;
...
class ImageDecoder {
	...
	private Options getBitmapOptionsForImageDecoding(ImageSize targetSize,
		...
		decodeOptions.inSampleSize = computeImageScale(targetSize, scaleType,
				viewScaleType);
	...
	public static Bitmap decodeFromFile(String file, int width, int height) {
		...
		int sample = 1;
		if (option.outHeight > height || option.outWidth > width) {

			if(option.outHeight>option.outWidth){
				sample =Math.round((float)option.outWidth/(float)width);
			}else{
				sample =Math.round((float)option.outHeight/(float)width);
			}
		}
		option.inJustDecodeBounds =false;
		option.inPreferredConfig =Config.RGB_565;
		option.inSampleSize =sample;
		return BitmapFactory.decodeFile(file, option);
	}
}